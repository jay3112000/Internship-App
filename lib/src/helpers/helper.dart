...
//        },
  ...
  static void load() async {
    ...
    userlat = double.parse(prefs1.getString('lat'));
    userlon = double.parse(prefs2.getString('lon'));
  ...
  static bool canDelivery(Market _market, {List<Cart> carts}) {
    ...
    if (_distance == 0 && !deliveryAddress.value.isUnknown()) {
      _distance = sqrt(
          pow(69.1 * (double.parse(_market.latitude) - userlat), 2) +
              pow(
                  69.1 *
                      (userlon - double.parse(_market.longitude)) *
                      cos(double.parse(_market.latitude) / 57.3),
                  2));
    }
    print('distance=$_distance');
    print('userlat=$userlat');
    print('range=$_deliveryRange');
