...
Future<Stream<Market>> getNearMarkets(
  ...
  try {
    ...
    var streamedRest = await client.send(http.Request('get', uri));
...
Future<List<dynamic>> getMarket2(String id) async {
  List<Map<String, dynamic>> array=[] ;
  String url = "https://demo.profdeve.sa/akhdar/public/api/markets/$id";
  ...
  await for (var contents in response.transform(Utf8Decoder())) {
    ...
    print("mapvalues $map");
    if (map['success'] == true) {
      array = [{'open_time' : map['data']['open_time']},{'close_time' : map['data']['close_time']},{'interval_time' : map['data']['interval_time']},{'brake_time' : map['data']['brake_time']}];
    } else {
      array = [{'error':'no value found'}];
    }
  }
  return array;
}
